...
public class ReportDto {
    // 데이터 존재 여부 (화면 '기록 없음' 표시용)
    private boolean hasData;

    // 건강 점수 추이 (X축: 날짜, Y축: 점수)
    private List<String> scoreDates;
    private List<Integer> scoreValues;
    private int averageScore;

    // BMI 및 체중 추이 (X축은 scoreDates 공유)
    private List<Double> bmiValues;    // 그래프 그리기용 (BMI)
    private List<Double> weightValues; // 툴팁 표시용 (체중)

    // 식단 레이더 차트 (순서: 탄,단,지,당,식이)
