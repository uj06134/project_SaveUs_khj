...
public class CommunityApiController {
    ...
    @GetMapping("/posts")
    public ResponseEntity<List<CommunityPostDto>> getFilteredPosts(
        ...
        try {
            // persona가 null이거나 ""이면 전체 조회, 값이 있으면 필터링 조회
            List<CommunityPostDto> postList = communityService.getPostList(userId, persona);
    ...
    public ResponseEntity<List<CommunityPostDto>> getPopularPosts(
        ...
        try {
            List<CommunityPostDto> popularList = communityService.getPopularPostList(userId, persona);
            return ResponseEntity.ok(popularList);
        } catch (Exception e) {
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(null);
        }
